...
API即**应用程序编程接口**。
API的作用是为不同的软件提供通信平台。利用API时，开发者只需要考虑如何向API发送请求与交换数据，而不需要考虑API内部发生了什么(真的不用吗？——某不愿透露姓名的SpongeMixin热爱者)。  
...
- Mojang Mappings：官方映射表，也是当前NeoForge的默认映射表，是目前最完善的映射表。但是其使用条款比较多
- Yarn：Fabric团队的映射表。Yarn更新快，但是质量略逊于Mojang Mappings
- Parchment：一个民间映射表

还有一个已经死了的MCP-Reborn映射

---

有了反混淆映射表后，我们还需要一系列注入工具，用于修改Minecraft本身的行为。这个工具就是**SpongeMixin**。  
SpongeMixin的作用是，允许开发者在程序启动之初，通过字节码机制直接修改Minecraft自身的行为。这里不需要太过纠结，知道就可以，Mixin属于很高级的概念了。
...
- 注入。对于更加深入的模组行为，有时候我们不得不采用注入。模组加载器一般都提供了简单的注入接口。
- 网络编程。

1.21以来，Mojang对Minecraft做出了一系列破坏性的更新。也就是说，旧版本的模组必须要大改才能移植到新版本。所以建议要紧跟版本更新。
...
随后将压缩包解压到一个你喜欢的目录(对，最好是全英文、无空格)，用IDEA打开，IDEA会自动识别Gradle项目并构建。
